...
def main(feature_set):
    ...
    my_svm = svm.SVC(C=1.0, kernel='linear', probability=True, class_weight=None)
    ...
    # grid_search = sklearn.grid_search.GridSearchCV( my_svm, search_parameters,
    #      cv=cv, verbose=VERBOSE_FLAG,n_jobs=NUMJOBS)
    grid_search = GridSearchCV(my_svm, search_parameters, cv=cv, verbose=VERBOSE_FLAG, n_jobs=NUMJOBS)
