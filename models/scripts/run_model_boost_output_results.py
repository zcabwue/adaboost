...
def main(feature_set):
    ...
    trees_model = sklearn.ensemble.ExtraTreesClassifier(n_jobs=NUMJOBS, min_samples_leaf=5, max_depth=3, max_features=None,
    ...
    ada_boost_model = GridSearchCV(AdaBoostClassifier(trees_model, n_estimators=NESTIMATORS), search_parameters,
                               cv=cv, verbose=VERBOSE_FLAG, n_jobs=NUMJOBS)
    # Fit model in grid search
    # with open ("../data/results_kks.csv", 'w') as csvfile:
    #     csv_writer = csv.writer(csvfile);
    #     csv_writer.writerow(["Docket", "Term", "Majority Votes", "Predicted", "Actual"])
    ...
    #              csv_writer.writerow([decided_cases.index[test_index[i]], decided_cases['term'][test_index[i]], decided_cases['majVotes'][test_index[i]],  results[i],  gs_results[test_index][i]])

    with open("../data/results_kks.csv", 'w', newline='') as csvfile:
        csv_writer = csv.writer(csvfile)
        csv_writer.writerow(["Docket", "Term", "Majority Votes", "Predicted", "Actual"])

        # Iterate over each split
        for train_index, test_index in cv.split(gs_cases):
            model = ada_boost_model.fit(gs_cases[train_index], gs_results[train_index])
            results = model.predict(gs_cases[test_index])
            for i in range(len(test_index)):
                index = test_index[i]
                csv_writer.writerow([
                    decided_cases.index[index],
                    decided_cases['term'][index],
                    decided_cases['majVotes'][index],
                    results[i],
                    gs_results[index]
                ])
